...
export default function Services() {
  return (
    <section id="services" className="py-24 px-6 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-px opacity-50"
      ...
      <div className="max-w-7xl mx-auto text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 reveal">
          {services.map((service) => (
            <div key={service.title} data-hover
              className="glass p-8 rounded-2xl group transition-all hover:border-opacity-100"
              style={{ '--hover-border': service.color }}
              onMouseEnter={(e) => (e.currentTarget.style.borderColor = service.color)}
              onMouseLeave={(e) => (e.currentTarget.style.borderColor = '')}>
              <div className="w-16 h-16 mx-auto rounded-full flex items-center justify-center mb-6 transition-all"
