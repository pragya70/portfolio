      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative">
          ...
          {experience.map((item, i) => (
            <div key={i}
              className={`relative flex flex-col ${item.right ? 'md:flex-row' : 'md:flex-row-reverse'} items-center justify-between mb-16 w-full reveal`}>
              {/* Spacer */}
              <div className="order-1 md:w-5/12" />
              {/* Icon dot */}
              <div className="z-20 flex items-center order-1 w-12 h-12 rounded-full justify-center flex-shrink-0"
                style={{ background: item.color }}>
                <i className={`${item.icon} text-black text-sm`} />
              </div>
              {/* Card */}
              <div className={`order-1 md:w-5/12 ${item.right ? 'text-left pl-8 md:pl-0 md:pr-8' : 'text-right pl-8 md:pl-8 md:pr-0'}`}>
                <div data-hover className={`glass p-6 rounded-xl border-l-4 ${!item.right ? 'md:border-l-0 md:border-r-4' : ''}`}
