...
export function Navbar({ shortName }: { shortName?: string }) {
  ...
  const displayName = (shortName ?? 'Pragya').toUpperCase();
  ...
  return (
    <>
      ...
      {/* Mobile overlay */}
      <div
        className={`fixed inset-0 z-30 flex flex-col items-center justify-center space-y-8 transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`}
      ...
      >
        {NAV_LINKS.map((l) => (
          <a
            ...
            className="text-2xl font-bold hover:text-[#00f3ff] transition-colors"
          >
            {l.label}
          </a>
        ))}
        <a
          href="#contact"
          onClick={() => setOpen(false)}
          className="text-2xl font-bold hover:text-[#00ff87] transition-colors"
        >
          CONTACT
        </a>
